...
module Make (Storage : MessageStorage.S) = struct
  ...
  module Field = struct
    ...
    let discriminantValue_get (x : 'cap t) : int =
      get_struct_field_uint16 ~default:0xffff x 2
  ...
  module Node = struct
    ...
    module Struct = struct
      ...
      let dataWordCount_get (x : 'cap t) : int =
        get_struct_field_uint16 x 14

      let pointerCount_get (x : 'cap t) : int =
        get_struct_field_uint16 x 24

      let preferredListEncoding_get (x : 'cap t) : ElementSize.t =
        let int_val = get_struct_field_uint16 x 26 in
        match int_val with
        | 0 -> ElementSize.Empty
        | 1 -> ElementSize.Bit
        | 2 -> ElementSize.Byte
      ...
      let isGroup_get (x : 'cap t) : bool =
        get_struct_field_bit x 28 0

      let discriminantCount_get (x : 'cap t) : int =
        get_struct_field_uint16 x 30
